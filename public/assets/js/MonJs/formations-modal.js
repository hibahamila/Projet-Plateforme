...
function checkIfFormationInCart(formationId) {
    // Récupérer les IDs des formations dans le panier depuis le localStorage
    const cartFormations = JSON.parse(localStorage.getItem('cartFormations') || '[]');
    
    // Convertir l'ID de formation en chaîne pour la comparaison
    const formationIdStr = formationId.toString();
    
    // Debug pour voir ce qui est comparé
    console.log("Vérification panier:", {
        formationIdToCheck: formationIdStr,
        cartFormations: cartFormations,
        formationInCart: cartFormations.includes(formationIdStr)
    });
    
    return cartFormations.includes(formationIdStr);
}
...
function createFormationCard(formation) {
    ...
    const isInCart = checkIfFormationInCart(formation.id);
    const cartButtonText = isInCart ? 'Accéder au panier' : 'Ajouter au panier';
    const cartButtonClass = isInCart ? 'btn-secondary' : 'btn-primary';
    const dataInCart = isInCart ? 'data-in-cart="true"' : '';
                                    ...
                                    <div class="product-box row">
                                        ...
                                        <div class="product-details col-lg-6 text-start">
                                            ...
                                            <div class="addcart-btn">
                                                <a class="btn ${cartButtonClass}" href="/panier" ${dataInCart}>${cartButtonText}</a>
